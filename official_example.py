...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--prompt", required=True, help="User prompt. If omitted, ask interactively."
    )
    parser.add_argument(
        "--image_path",
        default=None,
...
def save_data_url_to_file(data_url: str, path: Path) -> None:
    matched = re.match(
        r"data:image/(?P<subtype>[\w+.-]+);base64,(?P<b64>.+)", data_url, re.DOTALL
    )
    if not matched:
...
def save_images_from_message(
    message: dict[str, Any], out_dir: Path, prefix: str
) -> None:
    ...
    for idx, item in enumerate(images):
        ...
        if not image_url.startswith("data:image/"):
            continue
        ext = "png"
        if image_url.startswith("data:image/jpeg") or image_url.startswith(
            "data:image/jpg"
        ):
            ext = "jpg"
        ...
        "messages": [
            {"role": "system", "content": GENERATION_SYSTEM_PROMPT},
            {
                ...
                "content": [
                    {
                        "type": "image_url",
                        "image_url": {"url": local_image_to_data_url(args.image)},
                    },
...
def run_interleave_stream(
    args: argparse.Namespace, out_dir: Path, timestamp: str
) -> None:
    chat_url, headers = build_client(args.url, args.api_key)
    image_config = build_image_config(args)
    content = []
    ...
    response = requests.post(
        chat_url, headers=headers, json=payload, stream=True, timeout=600
    )
    response.raise_for_status()
...
def run_vqa(args: argparse.Namespace, out_dir: Path, timestamp: str) -> None:
    ...
    if args.image_path:
        content.append(
            {
                "type": "image_url",
                "image_url": {"url": local_image_to_data_url(args.image_path)},
            }
        )
    content.append({"type": "text", "text": args.prompt})
    payload = {
